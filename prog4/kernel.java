...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
		...
		// let the current thread sleep in waitQueue under the condition
		 ...
		 int my_tid = myTcb.getTid();
		 return waitQueue.enqueueAndSleep(my_tid);
		...
		int my_pid = myTcb.getPid();
		waitQueue.dequeueAndWakeup(my_pid, myTcb.getTid());
...
/* part2: modify Kernel.java to use SyncQueue.java in the three
case statements: RAWREAD, RAWWRITE, and SYNC, so that disk accesses will
no longer cause spin loops.*/
		...
		// Part2: for these changes to take effect, the code for the INTERRUPT_DISK casemust be enabled.
